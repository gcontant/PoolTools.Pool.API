using PoolTools.Pool.API.Domain.Common;
using System.Collections.Concurrent;
using System.ComponentModel;
using System.Globalization;

namespace PoolTools.Pool.API.Domain.Entities.TypeConverters;
    ...
    where TValue : notnull
{
    ...
    private static TypeConverter GetIdValueConverter()
    {
        ...
        if (!converter.CanConvertFrom(typeof(string)))
            throw new InvalidOperationException(
                $"Type '{typeof(TValue)}' doesn't have a converter that can convert from int");
        return converter;
    }
    ...
    public override object ConvertFrom(ITypeDescriptorContext context, CultureInfo culture, object value)
    {
        if (value is int i)
        {
            value = IdValueConverter.ConvertFrom(i)!;
        }
